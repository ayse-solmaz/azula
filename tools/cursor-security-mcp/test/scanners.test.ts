...
import { skipDirectory } from "../src/utils/fs.js";
...
describe("cursor-security scanners", () => {
  ...
  it("skips python virtualenv directory names", () => {
    assert.equal(skipDirectory(".venv"), true);
    assert.equal(skipDirectory(".venv-merge"), true);
    assert.equal(skipDirectory("site-packages"), true);
    assert.equal(skipDirectory("internal"), false);
  });
});
