...
export function scanBackend(files: SourceFile[]): Finding[] {
  ...
  for (const file of backendFiles) {
    ...
    while ((match = sqlConcat.exec(content)) !== null) {
      if (isMetaOrDocLine(lineAt(content, match.index)) || match[0].length > 160) continue;
      if (/\.go$/i.test(relativePath) && !/\b(SELECT|INSERT\s+INTO|UPDATE\s+\w+\s+SET|DELETE\s+FROM)\b/i.test(match[0])) {
        continue;
      }
      findings.push({
