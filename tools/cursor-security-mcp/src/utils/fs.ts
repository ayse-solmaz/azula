...
/** Skip Python/virtualenv trees including Azula's `.venv-merge`. */
export function skipDirectory(name: string): boolean {
  const n = name.toLowerCase();
  if (IGNORE_DIRS.has(name) || IGNORE_DIRS.has(n)) return true;
  if (n.startsWith(".venv") || n.startsWith("venv")) return true;
  if (n.endsWith(".egg-info")) return true;
  return false;
}
